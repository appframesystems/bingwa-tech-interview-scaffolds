...
export class TasksService {
  ...
  // Soft delete a task
  async softDelete(userId: string, taskId: string): Promise<TaskDocument> {
    const task = await this.findOne(userId, taskId);
  ...
  async restore(userId: string, taskId: string): Promise<TaskDocument> {
  ...
  async getStats(userId: string): Promise<any> {
    ...
    const total = await this.taskModel.countDocuments(filter);
    const todo = await this.taskModel.countDocuments({ ...filter, status: TaskStatus.TODO });
    const inProgress = await this.taskModel.countDocuments({ ...filter, status: TaskStatus.IN_PROGRESS });
    const completed = await this.taskModel.countDocuments({ ...filter, status: TaskStatus.COMPLETED });
    const highPriority = await this.taskModel.countDocuments({ ...filter, priority: 'HIGH' });
  ...
  // Get tasks by status
  async findByStatus(userId: string, status: TaskStatus): Promise<TaskDocument[]> {
